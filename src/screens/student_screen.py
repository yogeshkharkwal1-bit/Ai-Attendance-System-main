...
def student_dashboard():
    ...
    with c2:
        if st.button('Enroll in Subject', type='primary', use_container_width=True, key='enroll_dialog_trigger_btn'):
            enroll_dialog()
    ...
    for i, sub_node in enumerate(subjects):
        ...
        def unenroll_button_callback(subject_id=sid, index=i):
            if st.button(
                "Unenroll from this course", 
                type='tertiary', 
                use_container_width=True, 
                icon=':material/delete_forever:',
                key=f"unenroll_btn_{subject_id}_{index}" 
            ):
                unenroll_student_to_subject(student_id, subject_id)
                st.toast(f'Unenrolled from this subject successfully!')
                time.sleep(0.5)
                st.rerun()
        ...
        with cols[i % 2]:
            subject_card(
                ...
                footer_callback=unenroll_button_callback 
            )
